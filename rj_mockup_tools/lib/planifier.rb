    ...
    module Planifier
      ...
      def planify_type(model, instances_data, type_name, start_org, layer_name, material_info, mode, settings, apply_mirror: false, apply_scale: false)
        ...
        copied_with_info.each do |info|
          ...
          # PASSO 2.5: APLICA ESCALA NA ORIGEM (se necessário)
          # IMPORTANTE: Aplicar a escala ANTES das rotações evita erros de precisão
          if apply_scale
            # Aplica escala usando a ORIGEM como ponto de referência
            # Isso garante precisão máxima e evita erros acumulados
            ...
            puts "    ✓ Escala aplicada: X=#{scale_info[:scale_x].abs.round(10)}, Y=#{scale_info[:scale_y].abs.round(10)}, Z=#{scale_info[:scale_z].abs.round(10)}"

            # Move novamente para origem após escala
            bounds = ci.bounds
            offset_to_origin = Geom::Vector3d.new(-bounds.min.x, -bounds.min.y, -bounds.min.z)
            ci.transform! Geom::Transformation.translation(offset_to_origin)
          end
          ...
          # PASSO 7: Move para origem XY final
          bounds = ci.bounds
          if bounds.min.x.abs > 0.001 || bounds.min.y.abs > 0.001
            offset_final = Geom::Vector3d.new(-bounds.min.x, -bounds.min.y, 0)
            ci.transform! Geom::Transformation.translation(offset_final)
          end
          ...
          # VERIFICAÇÃO FINAL: Valida dimensões e posição
          final_bounds = ci.bounds
          puts "    📏 Dimensões finais: #{(final_bounds.width/1.mm).round(2)}x#{(final_bounds.height/1.mm).round(2)}x#{(final_bounds.depth/1.mm).round(2)}mm"
          puts "    📍 Posição min: (#{(final_bounds.min.x/1.mm).round(2)}, #{(final_bounds.min.y/1.mm).round(2)}, #{(final_bounds.min.z/1.mm).round(2)})"
